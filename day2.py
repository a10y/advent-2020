...
def check_policy_part1(line):
    # Read a fucking number
    print(line)
    policy, password = line.split(":")
    password = password.strip()

    # Parse the policy out
    start = int(policy[:policy.index("-")])
    end = int(policy[policy.index("-")+1:policy.index(" ")])
    letter = policy[-1]
    print((start, end, letter))

    count = sum([1 if let == letter else 0 for let in password])
    return count >= start and count <= end

def check_policy_part2(line):
    # Read a fucking number
    print(line)
    policy, password = line.split(":")
    password = password.strip()
    ...
    # Parse the policy out
    start = int(policy[:policy.index("-")]) - 1
    end = int(policy[policy.index("-")+1:policy.index(" ")]) - 1
    letter = policy[-1]
    return (password[start] == letter) ^ (password[end] == letter)

print(sum([1 if check_policy_part1(line) else 0 for line in policy.split("\n")]))
print(sum([1 if check_policy_part2(line) else 0 for line in policy.split("\n")]))
