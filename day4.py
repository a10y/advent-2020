...
def check_passport_part1(passport):
    return passport.keys() == set(["byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid", "cid"]) \
            or passport.keys() == set(["byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid"])

def check_passport_part2(passport):
    if passport.keys() != set(["byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid", "cid"]) \
            and passport.keys() != set(["byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid"]):
        return False
    try:
        import pdb
        #pdb.set_trace()
        ...
        if eyr < 2020 or eyr > 2030:
            return False
        if hgt_unit == "cm" and (hgt_amt < 150 or hgt_amt > 193):
            return False
        if hgt_unit == "in" and (hgt_amt < 59 or hgt_amt > 76):
            return False
        if hgt_unit not in set(["cm", "in"]):
            return False
        if hcl[0] != '#' or len(hcl) != 7 or not set(hcl[1:]).issubset(set("abcdef0123456789")):
            return False
        if ecl not in set(["amb", "blu", "brn", "gry", "grn", "hzl", "oth"]):
            return False
        if len(pid) != 9 or not set(pid).issubset("0123456789"):
            return False
...
def solve_part1():
    ...
    for passport in passports.split("\n\n"):
        ...
        passport = {it.split(":")[0]: it.split(":")[1] for it in items}
...
#solve_part1()
solve_part2()
#print(check_passport_part2({'ecl': 'blu', 'pid': '071991002', 'eyr': '2021', 'byr': '1978', 'cid': '321', 'hcl': '#efcc98', 'iyr': '2013', 'hgt': '68in'}))
