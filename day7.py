...
class Rule(object):
    def __init__(self, color, quantities):
        # How many combos are there from this fucking rule?
        self._color = color
        self._quantities = quantities

    def combos(self):
        # Generator that yields every possible combo for this fucking bag
        return
        yield

# Return the set of visited things...

def solve_part1():
    # Store the bags carried directly by another bag
    bag_types = {}

    for rule in rules.splitlines():
        color, contains = map(str.strip, rule.split("bags contain"))
        if contains == "no other bags.":
            bag_types[color] = []
        else:
            implied = contains.split(", ")
            bag_types[color] = [(int(imp.split()[0]), ' '.join(imp.split()[1:-1])) for imp in implied]
        
    # Generate every collection of bags
    ...
    def evaluate(rules, bag, visited=set()):
        if bag in visited:
            return
        ...
        for count, inner in rules[bag]:
            evaluate(rules, inner, visited)


    for color in bag_types.keys():
        print(f"trying {color}")
        visited = set()
        evaluate(bag_types, color, visited)
        if 'shiny gold' in visited:
            valid_colors.add(color)

    valid_colors = valid_colors - set(["shiny gold"])
    print(valid_colors)
...
def solve_part2():
    ...
    def evaluate(rules, bag, counts=dict()):
        counts[bag] += 1        
        ...
        for count, inner in rules[bag]:
            for _ in range(count):
                evaluate(rules, inner, counts)

    from collections import defaultdict
    counts = defaultdict(int)
    evaluate(bag_types, "shiny gold", counts)
